...
# -------------------------
# Direction shortcut tracker (Case 2: ignoring directions / ignoring anchors)
# -------------------------
class DirectionShortcutTracker:
    """
    Tracks two direction-based spurious correlation shortcuts at generation time.

    Case A — "Ignore ALL directions":
        The model can arrive at the correct answer by simply counting every
        target-attribute object in the entire scene, ignoring all directional
        constraints.  This is flagged when every target-attribute object in
        the scene happens to fall inside the valid intersection region (i.e.
        there are ZERO target-attribute objects outside the valid region).

    Case B — "Ignore anchor k" (depth >= 2 only):
        For each anchor k, the model could ignore anchor k and still get
        the correct answer.  This happens when the count of target-attribute
        objects inside the intersection of all OTHER anchors' regions (i.e.
        dropping anchor k's constraint) equals the count inside the full
        intersection of ALL anchors.  If equal, anchor k is redundant —
        the model can arrive at the correct answer without it.

    Both metrics are tracked per bucket and reported / saved to CSV.

    Required keys in `ex` (from generate_example):
        'all_scene_objects', 'ph_map', 'intersection_region',
        'per_anchor_valids', 'anchor_coords'
    """

    def __init__(self):
        # Case A: bucket -> {"total": N, "shortcut": M}
        self.ignore_dir = defaultdict(lambda: {"total": 0, "shortcut": 0})
        # Case B: bucket|anchor_key -> {"total": N, "shortcut": M}
        self.ignore_anchor = defaultdict(lambda: {"total": 0, "shortcut": 0})

    def _target_positions(self, all_objects, ph_map, qtype):
        """
        Return the set of (x,y) positions of ALL objects in the scene that
        match the target attribute (regardless of whether they are in the
        valid region).
        """
        positions = set()
        if qtype == 'SO':
            ...
            if not target_keys:
                return positions
            tshape = ph_map[target_keys[0]]
            for obj in all_objects:
                if obj['shape'].lower() == tshape.lower():
                    positions.add((obj['x'], obj['y']))
        elif qtype == 'CO':
            target_keys = [k for k in ph_map if k.startswith('color')]
            if not target_keys:
                return positions
            tcolor = ph_map[target_keys[0]]
            for obj in all_objects:
                if obj['color'].lower() == tcolor.lower():
                    positions.add((obj['x'], obj['y']))
        else:  # M, A, BTW
            ...
            for obj in all_objects:
                if obj['color'].lower() == tc.lower() and obj['shape'].lower() == ts.lower():
                    positions.add((obj['x'], obj['y']))
        return positions

    def record(self, ex, qtype, depth, form_idx, grid, dens):
        """Record one generated example."""
        # Skip A (no direction) and CMP (two independent sub-queries)
        if qtype in ('A', 'CMP'):
            return
        ...
        all_objects = ex.get('all_scene_objects', ex.get('objects', []))
        ph_map = ex.get('ph_map', {})
        intersection = ex.get('intersection_region', set())
        per_anchor = ex.get('per_anchor_valids', {})

        target_pos = self._target_positions(all_objects, ph_map, qtype)
        if not target_pos:
            return

        # Exclude anchor positions from target positions for the check
        anchor_coords = ex.get('anchor_coords', {})
        anchor_positions = set(anchor_coords.values())
        target_pos -= anchor_positions

        if not target_pos:
            return

        # --- Case A: ignore ALL directions ---
        self.ignore_dir[bucket_key]["total"] += 1
        # If every target-attribute object sits inside the valid intersection,
        # the model can ignore directions entirely.
        if target_pos <= intersection:
            self.ignore_dir[bucket_key]["shortcut"] += 1

        # --- Case B: ignore anchor k (only meaningful for depth >= 2) ---
        # The shortcut: can the model drop anchor k and still get the same
        # answer?  Compute the intersection of all OTHER anchors' valid
        # regions (without k), count target-attribute objects there, and
        # compare to the count in the full intersection.  If equal,
        # anchor k is redundant.
        if len(per_anchor) >= 2:
            # Count in full intersection (the correct answer for form0)
            full_count = len(target_pos & intersection)
            anchor_keys_list = list(per_anchor.keys())
            for drop_ak in anchor_keys_list:
                ba_key = f"{bucket_key}|drop={drop_ak}"
                self.ignore_anchor[ba_key]["total"] += 1
                # Intersect valid regions of all anchors EXCEPT drop_ak
                others_intersection = None
                for ak in anchor_keys_list:
                    if ak == drop_ak:
                        continue
                    if others_intersection is None:
                        others_intersection = set(per_anchor[ak])
                    else:
                        others_intersection &= per_anchor[ak]
                if others_intersection is not None:
                    # Count target objects in the reduced intersection
                    reduced_count = len(target_pos & others_intersection)
                    # If dropping anchor k doesn't change the count,
                    # anchor k is redundant — shortcut exists
                    if reduced_count == full_count:
                        self.ignore_anchor[ba_key]["shortcut"] += 1

    def report(self):
        """Print summary reports for both cases."""
        # Case A
        print("\n===== Direction Shortcut Report (Case A: ignore ALL directions) =====")
        print(f"{'Bucket':<40} {'Total':>8} {'Shortcut':>10} {'Ratio':>8}")
        print("-" * 70)
        total_a, short_a = 0, 0
        for bucket in sorted(self.ignore_dir.keys()):
            d = self.ignore_dir[bucket]
            ratio = d["shortcut"] / d["total"] if d["total"] > 0 else 0.0
            print(f"{bucket:<40} {d['total']:>8} {d['shortcut']:>10} {ratio:>8.4f}")
            total_a += d["total"]
            short_a += d["shortcut"]
        overall_a = short_a / total_a if total_a > 0 else 0.0
        print("-" * 70)
        print(f"{'OVERALL':<40} {total_a:>8} {short_a:>10} {overall_a:>8.4f}")
        print("=" * 70)

        # Case B
        if self.ignore_anchor:
            print("\n===== Direction Shortcut Report (Case B: ignore single anchor) =====")
            print(f"{'Bucket|Anchor':<50} {'Total':>8} {'Shortcut':>10} {'Ratio':>8}")
            print("-" * 80)
            total_b, short_b = 0, 0
            for ba_key in sorted(self.ignore_anchor.keys()):
                d = self.ignore_anchor[ba_key]
                ratio = d["shortcut"] / d["total"] if d["total"] > 0 else 0.0
                print(f"{ba_key:<50} {d['total']:>8} {d['shortcut']:>10} {ratio:>8.4f}")
                total_b += d["total"]
                short_b += d["shortcut"]
            overall_b = short_b / total_b if total_b > 0 else 0.0
            print("-" * 80)
            print(f"{'OVERALL':<50} {total_b:>8} {short_b:>10} {overall_b:>8.4f}")
            print("=" * 80)

    def save_csv(self, path_prefix):
        """
        Save both reports to CSV files.
        Creates: <path_prefix>_ignore_dir.csv  and  <path_prefix>_ignore_anchor.csv
        """
        import csv
        # Case A
        with open(f"{path_prefix}_ignore_dir.csv", 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["bucket", "total", "shortcut_count", "ratio"])
            for bucket in sorted(self.ignore_dir.keys()):
                d = self.ignore_dir[bucket]
                ratio = d["shortcut"] / d["total"] if d["total"] > 0 else 0.0
                writer.writerow([bucket, d["total"], d["shortcut"], f"{ratio:.4f}"])
        # Case B
        if self.ignore_anchor:
            with open(f"{path_prefix}_ignore_anchor.csv", 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["bucket_anchor", "total", "shortcut_count", "ratio"])
                for ba_key in sorted(self.ignore_anchor.keys()):
                    d = self.ignore_anchor[ba_key]
                    ratio = d["shortcut"] / d["total"] if d["total"] > 0 else 0.0
                    writer.writerow([ba_key, d["total"], d["shortcut"], f"{ratio:.4f}"])
...
def build_dataset(out_root: str):
    ...
    dir_tracker = DirectionShortcutTracker()
                    ...
                    for split, split_quota in quotas.items():
                        ...
                        while i < split_quota:
                            try:
                                ...
                                # --- Track spurious correlations ---
                                sp_tracker.record(ex, qtype, depth, idx, grid, dens)
                                dir_tracker.record(ex, qtype, depth, idx, grid, dens)
    ...
    dir_tracker.report()
    dir_tracker.save_csv(os.path.join(out_root, "direction_shortcut_report"))
