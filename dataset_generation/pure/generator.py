...
class DatasetGenerator:
    ...
    def sample_scene(self, ph_map, depth, qtype, idx=0):
        ...
        valids = []
        per_anchor_valids = {}   # {anchor_key: set of cells satisfying ONLY that anchor's constraint}
        target_coords = []
        ...
        for t in range(num_targets):
            ...
            for ak, dk in zip(anchor_keys, dir_keys):
                ...
                valid = valid & region
                # Store per-anchor valid region (cells satisfying this anchor's
                # directional constraint on the full grid, before intersection)
                full_grid = set((ci,cj) for ci in range(self.grid_size)
                                        for cj in range(self.grid_size))
                ak_region = set()
                for ci,cj in full_grid:
                    if direction=='left of'  and ci < x0: ak_region.add((ci,cj))
                    elif direction=='right of' and ci > x0: ak_region.add((ci,cj))
                    elif direction=='above'    and cj < y0: ak_region.add((ci,cj))
                    elif direction=='below'    and cj > y0: ak_region.add((ci,cj))
                per_anchor_valids[ak] = ak_region
        ...
        # --- Per-anchor "confuser" regions for direction-shortcut mitigation ---
        #
        # To prevent the model from ignoring anchor k, we need target-class
        # objects that satisfy every OTHER anchor's constraint but VIOLATE
        # anchor k's.  If the model drops anchor k, it would wrongly count
        # these objects, so the shortcut fails.
        #
        # confuser_region[ak] = intersection of all other anchors' valid
        #                       regions  MINUS  anchor k's valid region
        #
        # Depth 2 (A,B):  confuser[A] = valid_B \ valid_A
        #                 confuser[B] = valid_A \ valid_B
        # Depth 3 (A,B,C): confuser[A] = (valid_B & valid_C) \ valid_A  etc.
        confuser_regions = {}
        if qtype != 'CMP' and len(anchor_keys) >= 2:
            for drop_ak in anchor_keys:
                # Intersect valid regions of all anchors EXCEPT the dropped one
                others_valid = None
                for ak in anchor_keys:
                    if ak == drop_ak:
                        continue
                    if others_valid is None:
                        others_valid = set(per_anchor_valids[ak])
                    else:
                        others_valid &= per_anchor_valids[ak]
                # Subtract the dropped anchor's valid region
                if others_valid is not None:
                    confuser = others_valid - per_anchor_valids[drop_ak]
                    # Also remove used positions (anchors, targets already placed)
                    confuser -= used_positions
                    if confuser:
                        confuser_regions[drop_ak] = confuser

        # --- Guaranteed confuser placement ---
        # Before the filler loop, place exactly ONE target-class object in
        # each non-empty confuser region so every anchor is "protected" even
        # when filler slots are scarce (e.g. density 0.3, depth 3).
        if confuser_regions and qtype not in ('CMP',):
            if qtype == 'SO':
                guar_c, guar_s = random.choice(QuestionTemplates.COLORS), tshape
            elif qtype == 'CO':
                guar_c, guar_s = tcolor, random.choice(QuestionTemplates.SHAPES)
            else:
                guar_c, guar_s = tcolor, tshape
            for drop_ak, cregion in list(confuser_regions.items()):
                candidates = [cell for cell in cregion if cell not in used_positions]
                if candidates and more > 0:
                    pos = random.choice(candidates)
                    if qtype == 'SO':
                        guar_c = random.choice(QuestionTemplates.COLORS)
                    elif qtype == 'CO':
                        guar_s = random.choice(QuestionTemplates.SHAPES)
                    objects.append({'x': pos[0], 'y': pos[1],
                                    'color': guar_c, 'shape': guar_s})
                    used_positions.add(pos)
                    more -= 1  # consume one filler slot

        if qtype == 'CMP':
            tc, ts = ph_map[target_keys[0]].split()
            tc2, ts2 = ph_map[target_keys[1]].split()
        
        # For random distribution of target-class fillers across anchors
        confuser_list = list(confuser_regions.keys()) if confuser_regions else []
        ...
        for _ in range(more):
            ...
            if qtype != 'CMP' and outside:
                if random.random() <= 0.75:
                    (c, s) = (tcolor, tshape)
                    if qtype == 'SO':
                        (c, s) = random.choice([(col, tshape) for col in QuestionTemplates.COLORS])
                    elif qtype == 'CO':
                        (c, s) = random.choice([(tcolor, sh) for sh in QuestionTemplates.SHAPES])
                    # --- Per-anchor confuser constraint (depth >= 2) ---
                    # Place target-class filler in confuser_region[ak]:
                    # satisfies all OTHER anchors but violates anchor ak.
                    # This makes "ignore anchor ak" give wrong counts.
                    if confuser_list:
                        # Pick which anchor to "confuse" (random)
                        chosen_ak = random.choice(confuser_list)
                        # Fresh candidates (remove positions used since we built the set)
                        candidates = [cell for cell in confuser_regions[chosen_ak]
                                      if cell not in used_positions and cell != (i, j)]
                        if candidates:
                            # Release the already-claimed (i,j) and resample
                            used_positions.discard((i, j))
        ...
        return objects, anchor_coords, target_coords, valid, valids, cmp, per_anchor_valids
    ...
    def generate_example(self, depth, qtype, idx=0):
        ...
        objects, anchor_coords, target_coords, intersection_region, regions, cmp, per_anchor_valids = self.sample_scene(ph_map, depth, qtype, idx)
        ...
        return {
            ...
            'per_anchor_valids': per_anchor_valids,
            'answer': answer,
            'masks': masks
        }
...
if __name__ == '__main__':
    ...
    dg = DatasetGenerator(grid_size=5, density=0.3)
    ...
    # print(example['masks'])
    # example['image'].show()
    ph_map ={'target1': 'blue circle', 'anchor1': 'red triangle', 'target2': 'green square', 'anchor2': 'blue triangle', 'direction1': 'left of', 'direction2': 'right of'}
    qtype = 'CMP'
    objects, anchor_coords, target_coords, valid, regions, cmp, per_anchor_valids = dg.sample_scene(ph_map, 2, qtype, 0)
